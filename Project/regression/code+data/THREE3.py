...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden1_nodes,hidden2_nodes,hidden3_nodes,hidden4_nodes, hidden5_nodes, output_nodes, learning_rate):
        # Set number of nodes in input, hidden and output layers.
        self.input_nodes = input_nodes
        self.hidden1_nodes = hidden1_nodes
        self.hidden2_nodes=hidden2_nodes
        self.hidden3_nodes=hidden3_nodes
        self.hidden4_nodes=hidden4_nodes
        self.hidden5_nodes=hidden5_nodes
        ...
        self.weights_hidden3_to_hidden4=np.random.normal(size=(hidden4_nodes,hidden3_nodes+1))
        self.weights_hidden4_to_hidden5=np.random.normal(size=(hidden5_nodes,hidden4_nodes+1))
        self.weights_hidden5_to_output =np.random.normal(size=(output_nodes,hidden5_nodes+1))
        '''
    ...
    def train(self, inputs_list,iterator=12000):
        ...
        for it in range(iterator):
            ...
            hidden4_inputs=self.weights_hidden3_to_hidden4*hidden3_outputs
            hidden4_outputs=self.activation_function(hidden4_inputs)
            hidden4_outputs=np.row_stack((hidden4_outputs,ones((1,shape(hidden4_outputs)[1]))))

            hidden5_inputs=self.weights_hidden4_to_hidden5*hidden4_outputs
            hidden5_outputs=self.activation_function(hidden5_inputs)
            hidden5_outputs=np.row_stack((hidden5_outputs,ones((1,shape(hidden5_outputs)[1]))))

            final_inputs =self.weights_hidden5_to_output*hidden5_outputs# signals into final output layer
            ...
            EW4=self.weights_hidden5_to_output[:,:-1].T*output_errors
            der4=multiply(hidden5_outputs,1-hidden5_outputs)[:-1,:]
            hidden4_errors=multiply(der4,EW4)

            EW3=self.weights_hidden4_to_hidden5[:,:-1].T*hidden4_errors
            der3=multiply(hidden4_outputs,1-hidden4_outputs)[:-1,:]
            hidden3_errors=multiply(der3,EW3)

            EW2=self.weights_hidden3_to_hidden4[:,:-1].T*hidden3_errors
            ...
            hidden4_grad=output_errors*hidden5_outputs.T
            hidden3_grad=hidden4_errors*hidden4_outputs.T
            hidden2_grad=hidden3_errors*hidden3_outputs.T
            ...
            self.weights_hidden5_to_output +=self.lr*hidden4_grad/len(inputs)
            self.weights_hidden4_to_hidden5+=self.lr*hidden3_grad/len(inputs)
            self.weights_hidden3_to_hidden4+=self.lr*hidden2_grad/len(inputs)
...
m,n=shape(trainMat)
NN=NeuralNetwork(n,50,50,50,50,50,1,0.01)  
cnt,pre=NN.train(trainMat,12000)
print cnt*1.0/m
print pre
final_outputs=NN.run(testMat)
out=final_outputs[0].tolist()[0]
print out
np.savetxt("22_v3.csv", out, fmt="%d", delimiter="\n", newline="\n")
plt.figure("fg3")
plt.plot(ax,ay)
plt.show()