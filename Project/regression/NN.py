...
class NeuralNetwork(object):
    ...
    def train(self, inputs_list,iterator=10000):
        ...
        for it in range(iterator):
            ...
            final_errors = result - daan
            cntt=final_errors*final_errors.T
            rmse=np.sqrt(cntt*1.0/742)
            print mse, rmse,
            if rmse < min_rmse:
                min_rmse = rmse
                best_result = result
...
#     if i < 0:
#         i = 0
np.savetxt("22_v6.csv", br, fmt="%d", delimiter="\n", newline="\n")